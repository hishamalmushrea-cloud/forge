#include "sha256.h"
#include <string.h>

static const uint32_t K[64] = {
    0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
    0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
    0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
    0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
    0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
    0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
    0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
    0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
};

#define RR(x,n) (((x)>>(n))|((x)<<(32-(n))))
#define S0(x) (RR(x,7)^RR(x,18)^((x)>>3))
#define S1(x) (RR(x,17)^RR(x,19)^((x)>>10))
#define E0(x) (RR(x,2)^RR(x,13)^RR(x,22))
#define E1(x) (RR(x,6)^RR(x,11)^RR(x,25))
#define CH(x,y,z) (((x)&(y))^((~(x))&(z)))
#define MJ(x,y,z) (((x)&(y))^((x)&(z))^((y)&(z)))

static void block(const uint8_t *p, uint32_t h[8]) {
    uint32_t w[64], a, b, c, d, e, f, g, hh, t1, t2;
    for (int i = 0; i < 16; i++)
        w[i] = ((uint32_t)p[4*i]<<24)|((uint32_t)p[4*i+1]<<16)|((uint32_t)p[4*i+2]<<8)|p[4*i+3];
    for (int i = 16; i < 64; i++) w[i] = S1(w[i-2]) + w[i-7] + S0(w[i-15]) + w[i-16];
    a=h[0];b=h[1];c=h[2];d=h[3];e=h[4];f=h[5];g=h[6];hh=h[7];
    for (int i = 0; i < 64; i++) {
        t1 = hh + E1(e) + CH(e,f,g) + K[i] + w[i];
        t2 = E0(a) + MJ(a,b,c);
        hh=g;g=f;f=e;e=d+t1;d=c;c=b;b=a;a=t1+t2;
    }
    h[0]+=a;h[1]+=b;h[2]+=c;h[3]+=d;h[4]+=e;h[5]+=f;h[6]+=g;h[7]+=hh;
}

void sha256(const uint8_t *msg, uint32_t len, uint8_t out[32]) {
    uint32_t h[8] = {0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19};
    uint32_t i = 0;
    for (; i + 64 <= len; i += 64) block(msg + i, h);
    uint8_t tail[128];
    uint32_t rem = len - i;
    memcpy(tail, msg + i, rem);
    tail[rem++] = 0x80;
    size_t pad = (rem <= 56) ? (56 - rem) : (120 - rem);
    memset(tail + rem, 0, pad);
    rem += (uint32_t)pad;
    uint64_t bits = (uint64_t)len * 8;
    for (int j = 7; j >= 0; j--) { tail[rem++] = (uint8_t)(bits >> (j * 8)); }
    for (i = 0; i < rem; i += 64) block(tail + i, h);
    for (i = 0; i < 8; i++) {
        out[4*i]=(uint8_t)(h[i]>>24); out[4*i+1]=(uint8_t)(h[i]>>16);
        out[4*i+2]=(uint8_t)(h[i]>>8); out[4*i+3]=(uint8_t)h[i];
    }
}

void hmac_sha256(const uint8_t *key, uint32_t klen,
                 const uint8_t *msg, uint32_t mlen, uint8_t out[32]) {
    uint8_t kb[64], io[64], oo[64], inner[32];
    if (klen > 64) { sha256(key, klen, inner); key = inner; klen = 32; }
    memset(kb, 0, 64);
    memcpy(kb, key, klen);
    for (int i = 0; i < 64; i++) { io[i] = kb[i] ^ 0x36; oo[i] = kb[i] ^ 0x5c; }
    uint8_t buf[64 + 256];
    memcpy(buf, io, 64);
    memcpy(buf + 64, msg, mlen);
    sha256(buf, 64 + mlen, inner);
    memcpy(buf, oo, 64);
    memcpy(buf + 64, inner, 32);
    sha256(buf, 96, out);
}
